...
| Generic mitigation | toggle a flag back, restart a service, return a deployment to the revision it was running before | Autonomous, but announced in Slack immediately + logged, with whatever it left to put back recorded |
...
written to the repository the revision came from. One action answers both
deployment modes, because a revision carries the code and the configuration it
shipped with and the platform returns both together. That is also why it
mitigates without resolving - the repository still holds the change that caused
the incident, whether that change is a value or a line of code, and what stops
it being re-applied is the platform's own reconciliation staying suspended.
Writing to the repository instead would be an infrastructure change, which is a
tier up and a human's to approve.
