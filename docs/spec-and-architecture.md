...
**The three channels:**
1. `get_metrics_summary(window)` - a Prometheus range query, pre-aggregated buckets. Each minute carries the rate signals - error rate, p50/p95 latency, request volume - the resource ones: memory in use, the limit it is allowed, and when the serving process started - and how much of the minute a cache in front of the work carried, absent rather than zero where the deployment has no cache, since a service without one and a cache nothing reaches are different facts. Departure is judged on four of them: the error rate, both latencies, and the heap. The median is there because an incident can live entirely in it - a cache that stops answering moves the middle of the distribution while the tail, which always described the slow path, barely stirs. Cheap, small, called first: it locates the onset the other two anchor on.
