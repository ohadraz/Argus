# eval-sampling Specification

## Purpose
How a paid eval takes its samples and what may be concluded from them: a run is a batch rather than a population, every sample is recorded so batches pool, and a threshold is re-derived only from a pool deep enough to mean something.
## Requirements
