# config-rollback-mitigation Specification

## Purpose
Returning a deployment to a configuration revision it already ran: the failure
mode that calls for it, the tool that performs it through the platform's own
rollback rather than by writing to a repository, and what such a change leaves
behind for a withdrawal to put back. It mitigates and does not resolve - the
repository still holds what caused the incident.
## Requirements
