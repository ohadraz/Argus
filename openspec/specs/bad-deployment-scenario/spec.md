# bad-deployment-scenario Specification

## Purpose
The Target Service staging an incident whose cause is the revision that is
running: a deploy ships a figure derived the expensive way, on the path every
request takes, so the median, the 95th and the 99th all climb together while
nothing fails. No log line mentions a release, so the deploy history is the only
evidence naming a cause - the mirror of the scenarios that hide, where detection
is trivial and attribution is the whole difficulty.
## Requirements
