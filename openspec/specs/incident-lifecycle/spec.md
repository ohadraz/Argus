...
### Requirement: Every FSM transition is accounted for by a published event
The system SHALL publish a `StatusChanged` event (spec §11.1) for every
`Incident.status` transition, written on the same connection as the status
update, per the Orchestrator's single-writer rule.

Work that is worth recording and did not move the incident SHALL be published
by the node that did it, as the event that says what it was - an action refused
at the tier gate, the walk moving on to the next candidate, a change put back
by a withdrawal. Such an event SHALL NOT touch `Incident.status`, and SHALL NOT
be a status change reporting the status the incident is already in.

There SHALL be one account of an incident. Nothing writes a second record of
the same work in columns beside it, and every consumer - the page, the Slack
relay, the postmortem - reads that one account through the same renderer.

#### Scenario: Transition is accounted for
- **GIVEN** an incident currently in `investigating` status
- **WHEN** the graph transitions it to `mitigating`
- **THEN** a `StatusChanged` event to `mitigating` is published for that incident

#### Scenario: Work that settles nothing is published without a transition
- **GIVEN** an incident in `mitigating` status whose proposed action is refused
  at the tier gate
- **WHEN** the gate refuses the action
- **THEN** an `ActionRefused` event naming the candidate and the refusal is
  published, and `Incident.status` is unchanged
