...
`get_log_lines(window, filters)` tool that fetches the Target Service's
current log via HTTP and returns it as a list of log lines.
...
- **WHEN** `get_log_lines` is called with no `window` or `filters`
...
(`get_log_lines(window, filters) -> list[str]`) that performs a real MCP call
