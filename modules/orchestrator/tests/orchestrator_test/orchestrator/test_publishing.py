...
def test_an_alert_nobody_is_listening_for_is_still_received() -> None:
    # The account is never part of the work. Acknowledging with nobody
    # listening has to be as ordinary as acknowledging with a subscriber,
    # because the default everywhere else in this system is that nobody is.
    dont_care_alert = Alert(service="io-shop", alert_name="HighErrorRate")

    acknowledge_alert(_SOME_INCIDENT_ID, dont_care_alert, nobody)
...
def _the_investigator_runs(publisher: Any,
    ...
    candidate = a_determined_hypothesis(_SOME_INCIDENT_ID)
    ...
    def investigate(alert: Alert,
        ...
        return Findings(candidates=[candidate], already_read=[])
...
def _an_action_is_proposed(flag_changes: Any, publisher: Any) -> dict[str, Any]:
    ...
    state = state.model_copy(update={
        "hypothesis": a_determined_hypothesis(_SOME_INCIDENT_ID)
    })
