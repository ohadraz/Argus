...
def test_the_investigation_hands_the_walk_its_candidates(
    ...
    incident_id = a_random_id()
    the_best_answer = a_determined_hypothesis(incident_id)
    a_runner_up = a_determined_hypothesis(incident_id)
    investigate.return_value = _findings_of(the_best_answer, a_runner_up)
...
def test_a_resumed_investigation_is_told_what_was_read_and_what_failed(
    ...
    # answered.
    incident_id = a_random_id()
    investigate.return_value = _findings_of(a_determined_hypothesis(incident_id))
    a_refuted_attempt = _an_attempt_on(SOME_FLAG)
    a_window_already_read = Reading(
    ...
    a_second_round = _an_incident_being_investigated(incident_id).model_copy(
        update={"already_read": [a_window_already_read], "attempts": [a_refuted_attempt]}
    )
...
def test_a_refuted_candidate_hands_over_to_the_next_one(post_update: MagicMock) -> None:
    incident_id = a_random_id()
    the_refuted_candidate = a_determined_hypothesis(incident_id)
...
def test_what_was_tried_is_remembered_for_the_round_after(post_update: MagicMock) -> None:
    ...
    incident_id = a_random_id()
    a_candidate = a_determined_hypothesis(incident_id)
...
def test_a_spent_list_buys_another_investigation(post_update: MagicMock) -> None:
    ...
    # refuted.
    incident_id = a_random_id()
    the_only_candidate = a_determined_hypothesis(incident_id)
...
def _findings_of(*candidates: Hypothesis) -> agent_investigator.Findings:
    return agent_investigator.Findings(candidates=list(candidates), already_read=[])
