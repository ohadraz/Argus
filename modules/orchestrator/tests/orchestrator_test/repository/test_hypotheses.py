...
@pytest.mark.integration
def test_the_best_candidate_of_a_verdict_is_the_one_returned() -> None:
    # A verdict carries its runners-up, and each candidate is written in its
    # own transaction, so the newest *row* of a verdict is its weakest - the
    # alternative that names no cause at all.
    #
    # "Latest" has to mean the newest round's best answer, not the newest row.
    # A reader handed the alternative is told the investigation determined
    ...
    with psycopg.connect(DATABASE_URL) as conn:
        ...
        Scenario() \
            ...
            .then(
                the_stored_hypothesis_is(the_best_candidate)
            )
...
def _an_alternative_within(incident_id: str, rank: int) -> Hypothesis:
    """A runner-up of the same verdict - a competing account of one evidence set.

    Ranked below the answer it accompanies and written after it, as a verdict's
    candidates are, which is exactly what makes it the newest row and the wrong
    answer to "the latest hypothesis".
    """
    return Hypothesis(
        incident_id=incident_id,
