...
def test_candidates_formed_in_two_rounds_are_two_lines() -> None:
    ...
    a_candidate_a_refutation_and_another: list[IncidentEvent] = [
        _a_hypothesis(summary="the flag", rank=1, incident_id=some_incident),
        VerdictReached(
            incident_id=some_incident, hypothesis_id=new_id(), outcome="refuted"
        ),
...
def test_the_onset_line_points_at_the_minute_it_named() -> None:
    ...
    # work the link saves. A link elsewhere - "show the metrics" beside a line
    # that just read the metrics - is furniture, and there is none.
    some_onset = OnsetDetected(incident_id=new_id(), onset=SOME_MINUTE)

    Scenario() \
        .given(some_onset) \
        .when(lambda: build_narration([some_onset])) \
        .then(_the_only_line_points_at(f"#minute-{SOME_MINUTE}"))


@pytest.mark.unit
def test_an_action_line_points_at_the_change_it_reverted() -> None:
    # The action names one flag, so it can point at that flag's own row - and
    # the move is shown the way the flag table shows one, so a change reads as
    # a change wherever it appears.
    ...
    Scenario() \
        ...
        .then(all_of(_the_only_line_points_at(f"#flag-{SOME_FLAG}"),
...
def test_a_status_change_marks_the_status_it_moved_to() -> None:
    ...
    Scenario() \
        ...
        .then(all_of(_the_only_line_marks(str(IncidentStatus.MITIGATING).upper()),
                     _the_only_line_is_dressed_as(
                         f"moved-to {IncidentStatus.MITIGATING}")))
...
def _the_only_line_points_at(expected: str) -> Assertion[list[NarrationLine]]:
    def assertion(narration: list[NarrationLine]) -> bool:
        line = _the_only(narration)

        if line.link_target != expected:
            raise AssertionError(
                f"expected a link to [{expected}], got [{line.link_target}]"
            )

        if not line.link_label:
            raise AssertionError("expected the link to say what it leads to, it said nothing")
...
def _the_only_line_is_dressed_as(expected: str) -> Assertion[list[NarrationLine]]:
    def assertion(narration: list[NarrationLine]) -> bool:
        line = _the_only(narration)
