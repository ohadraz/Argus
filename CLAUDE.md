...
- `uv run python -m nox -s guard_layering` - import-linter over the contracts in the root `pyproject.toml`: `argus_core` depends on nothing in this workspace, `argus_incidents` knows only the kernel, `argus_web` serves HTML without installing an agent. A broken contract prints the import chain that broke it
...
- `uv run python -m nox -s e2e_replay` - the same stack and the same e2e tests, but the worker points at the Anthropic double, so every model answer is replayed from a committed recording. Free, keyless, and what CI runs on every push. Proves the pipeline works; proves nothing about whether the model was right
...
- **A package's `__init__.py` is an export list, never code.** Docstring, imports and `__all__` - no `def`, no `class`, no constant, no type alias. The entry point is not an exception: `write_postmortem` lives in `agent_postmortem/writing.py` and is re-exported from there. See the `package-init-is-exports-only` skill. `agent_codefix` still breaks this and is yet to be moved.
