...
- **A package's `__init__.py` is an export list, never code.** Docstring, imports and `__all__` - no `def`, no `class`, no constant, no type alias. The entry point is not an exception: `write_postmortem` lives in `agent_postmortem/writing.py` and is re-exported from there. See the `package-init-is-exports-only` skill. `agent_communicator` and `agent_codefix` still break this and are yet to be moved.
